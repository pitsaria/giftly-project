...
export class CheckoutPage implements OnInit {
  ...
  readonly minDate = new Date(Date.now() + CheckoutPage.PROCESSING_DAYS * 86400000).toISOString().substring(0, 10);
  readonly minTime = CheckoutPage.OPEN_TIME;
  readonly maxTime = CheckoutPage.CLOSE_TIME;
  deliveryDate = this.minDate;
  deliveryTime = CheckoutPage.OPEN_TIME;
  giftMessage = '';
  ...
  async placeOrder(): Promise<void> {
    ...
    if (this.deliveryDate && this.deliveryDate < this.minDate) {
      missing.push(`Delivery Date (at least ${CheckoutPage.PROCESSING_DAYS} days out)`);
    }
